...
The exemption is deliberately narrow, because "the character left" is an excuse that would otherwise cover a completely broken shot. A `blank_frames` kind was added alongside it so an empty or black clip has its own fatal channel rather than having to arrive as `character_missing` — the one verdict the exemption now waves through — and the prompt states outright that a scripted exit never explains a vanished setting or every character disappearing at once. **The action excuses where a character is and what is on the set; never who they are or what they are wearing.** e03 shipped with every clip passing and each verdict citing the exit as its justification, which is what a too-broad exemption buys you: not just a softer critic but a ready-made rationalization for the exact failure in front of it.

**Costume loss is `character_replaced`, not degradation.** The minor/fatal boundary kept collapsing here — the critic graded a clothed-to-nude change `fatal` on one attempt and "only costume colour/detail drift" on the next, and the second one shipped. `quality_degraded` now explicitly covers a character's *surface* ("the clay looks softer") and never their costume, with the instruction that if you find yourself describing a costume change as drift, it is `character_replaced`. Garment loss, a hat or hood changing shape or colour, and facial hair appearing or vanishing are all named as fatal, because "gradually" is exactly how a video model does it and gradual was reading as minor.
...
Tradeoff: clips 2+ within a segment lose the hero-anchor since they chain from the previous clip's last frame. Each segment re-anchors to the hero (or hero variant) at clip 0, so drift can't compound across segments — but it very much compounds *within* one, and the earlier claim here that "segments rarely need more than 3 chained clips" was treating that as acceptable. It isn't: Ubu Rex e03 had a 4-clip segment whose costumed puppet arrived at the final clip nude, recoloured, and mid-tantrum. **Every chain hop is a generation of drift**, so the number of clips per segment is a quality ceiling, not just a cost one. `adapt_play.py`'s `DEFAULT_SEGMENT_TARGET` is derived from `CLIP_TARGET_SECONDS` to hold segments to 2 clips (one hop); raising `--segment-target` buys longer unbroken shots and pays for them in drift.

**Stage directions are scoped to the clip that covers them** (`build_staged_action`, `scope_visual_prompt`). A direction annotates one turn and describes something that happens once, but a segment is several clips — so handing the whole segment's directions to every clip tells the model to perform a one-time action repeatedly. Worse, since clip M+1 chains from clip M's last frame, the next clip *starts* from the post-action state and has to reset in order to do it again. e03 segment 1 carried "PERE UBU going out, slamming the door" into all four clips: he exited four times, and the door Veo invented to satisfy the direction was baked into every chain anchor until it dominated the composition. Scoping uses the same `turn_timings` windowing as `build_speech_schedule`; a direction with no timings goes to the final clip, since an unplaceable direction is likelier to close a beat than open it and one clip performing it is right where every clip performing it is wrong.
